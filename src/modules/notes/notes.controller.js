...
const getAllNotes = async (req, res) => {
    ...
    if (req.user.role === 'Admin') {
            ...
            notes = await noteModel.find({ author: req.user.id }).select("title description ").populate({
                path: "author",
...
const deleteNote = async (req, res) => {
    let deletedNote = await noteModel.findOneAndDelete({ _id: req.params.id, author: req.user.id })
    if (deletedNote) {
